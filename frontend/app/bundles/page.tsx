...
export default function BundlesPage() {
  ...
  return <main className="max-w-6xl mx-auto px-4 py-10"><div className="flex items-end justify-between gap-4 mb-8"><div><h1 className="text-3xl font-black text-slate-900 flex items-center gap-3"><Layers3 className="w-8 h-8 text-[#1F1F1F]"/>Gói combo tiết kiệm</h1><p className="text-slate-600 mt-2">Mua nhiều dịch vụ cùng lúc với mức giá ưu đãi.</p></div><Link href="/cart" className="text-[#1F1F1F] font-semibold">Xem giỏ hàng</Link></div>{message&&<p className="mb-5 p-3 bg-blue-50 text-[#1F1F1F] rounded-sm">{message}</p>}{loading?<div className="py-20 flex justify-center"><Loader2 className="animate-spin text-[#1F1F1F]"/></div>:items.length===0?<div className="border rounded-sm p-10 text-center text-slate-600">Chưa có combo hoạt động. Admin có thể tạo qua API quản trị.</div>:<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">{items.map(item=><article key={item.id} className="bg-white border border-slate-200 rounded-sm overflow-hidden"><div className="p-6"><span className="inline-block px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-full text-xs font-bold">Giảm {item.discountPercent}%</span><h2 className="text-xl font-bold mt-4">{item.name}</h2><p className="text-sm text-slate-600 mt-2 min-h-10">{item.description}</p><p className="text-xs text-slate-600 mt-4">{JSON.parse(item.includedPlanIdsJson||'[]').length} dịch vụ trong combo</p><button type="button" onClick={()=>add(item.id)} disabled={busy===item.id} className="mt-5 w-full bg-blue-600 text-white rounded-sm py-3 font-semibold flex items-center justify-center gap-2">{busy===item.id?<Loader2 className="w-4 h-4 animate-spin"/>:<ShoppingCart className="w-4 h-4"/>}Thêm toàn bộ vào giỏ</button></div></article>)}</div>}</main>;
}
