    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-indigo-950 via-purple-950 to-slate-900 text-slate-900 py-20 sm:py-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="max-w-3xl">
            ...
            <a href="#pricing" className="inline-flex items-center gap-2 px-8 py-4 rounded-md bg-gradient-to-r from-purple-500 to-indigo-600 text-slate-900 font-bold shadow-xl hover:shadow-2xl transition-all">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="bg-slate-50 rounded-md p-6 border border-slate-100 text-center hover:shadow-md transition-all">
                <div className={`w-14 h-14 rounded-md ${f.color} flex items-center justify-center mx-auto mb-4`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            ...
            <div className="inline-flex bg-white rounded-full p-1 border border-slate-200 shadow-sm mt-4">
              <button onClick={() => setIsYearly(false)} className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${!isYearly ? 'bg-white text-slate-900' : 'text-slate-600'}`}>
                Hàng Tháng
              </button>
              <button onClick={() => setIsYearly(true)} className={`px-5 py-2 rounded-full text-sm font-bold transition-all flex items-center gap-1.5 ${isYearly ? 'bg-indigo-600 text-white' : 'text-slate-600'}`}>
                <span>Hàng Năm</span>
      ...
      <section className="py-16 bg-gradient-to-r from-indigo-600 to-purple-600 text-slate-900 text-center">
        <div className="max-w-3xl mx-auto px-4">
          ...
          <p className="text-slate-700 mb-8">Đăng ký hosting chỉ mất 2 phút. Hỗ trợ chuyển dữ liệu miễn phí.</p>
          <Link href="/" className="inline-flex items-center gap-2 px-8 py-4 rounded-md bg-white text-[#1F1F1F] font-bold shadow-xl hover:shadow-2xl transition-all">
            <Zap className="w-5 h-5" />
            Đăng Ký Hosting
          </Link>
        </div>
