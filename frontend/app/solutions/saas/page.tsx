...
import { Layers, ArrowRight, CheckCircle2, Maximize, TrendingUp, Zap } from 'lucide-react';
...
export default function SaasSolutionPage() {
  return (
    <div className="min-h-screen bg-emerald-50/30 overflow-hidden">
      {/* HERO */}
      <section className="pt-24 pb-20 bg-white border-b border-emerald-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <ScrollReveal animation="fade">
            <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-100 text-emerald-700 text-xs font-bold uppercase mb-6">
              <Layers className="w-4 h-4" /> SaaS Providers
            </span>
      ...
      <section className="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <ScrollReveal animation="fade">
          <h2 className="text-3xl font-black mb-12">Hệ sinh thái SaaS Providers</h2>
        </ScrollReveal>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[300px]">
          ...
          {/* Card 2 */}
          <div className="relative rounded-3xl overflow-hidden bg-emerald-500 group flex items-center justify-center p-8 text-center text-white">
          ...
          <div className="relative rounded-3xl overflow-hidden bg-white border border-slate-200 p-8 shadow-xl group">
             <div className="text-emerald-600 font-bold mb-4">THỐNG KÊ</div>
             <div className="text-6xl font-black text-slate-900 mb-2 group-hover:text-emerald-600 transition-colors">99.99%</div>
          ...
          <div className="md:col-span-2 relative rounded-3xl overflow-hidden bg-slate-100 group p-8 flex items-center justify-between">
            <div>
              ...
              <div className="text-emerald-600 font-mono"><TypewriterText text="Auto-scaling: ACTIVE" speed={40} delay={1}/></div>
            </div>
            <TrendingUp className="w-20 h-20 text-slate-300 group-hover:text-emerald-500 transition-colors" />
          </div>
        </div>
      </section>
         ...
         <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <StaggerContainer className="grid md:grid-cols-2 gap-12">
               <StaggerItem className="space-y-6">
                 ...
                 {[1,2,3,4,5].map(i => (
                   <div key={i} className="flex items-start gap-4 bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                     <CheckCircle2 className="w-6 h-6 text-emerald-500 shrink-0" />
                     <div>
                       <div className="font-bold">Module {i} cao cấp</div>
                       <div className="text-sm text-slate-600">Được tinh chỉnh đặc biệt cho SaaS Providers.</div>
                     </div>
                   </div>
                 ))}
               </StaggerItem>
               <StaggerItem className="bg-slate-900 rounded-3xl p-10 text-white flex flex-col justify-center">
                 ...
                 <Link href="/contact" className="py-4 bg-emerald-500 text-white text-center font-bold rounded-xl hover:bg-emerald-600 transition-colors">
                    Triển Khai Ngay
                 </Link>
               </StaggerItem>
            </StaggerContainer>
