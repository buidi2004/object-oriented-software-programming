...
import { ShieldCheck, ArrowRight, ArrowRightCircle } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-100 overflow-hidden">
      ...
      <section className="pt-32 pb-24 text-center max-w-5xl mx-auto px-4">
        <ScrollReveal animation="slide-up">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-slate-200 text-slate-700 font-bold uppercase tracking-wider mb-8 shadow-sm">
             <ShieldCheck className="w-5 h-5 text-red-500" /> Giải pháp tối ưu
          </div>
           ...
           <StaggerContainer className="flex flex-wrap lg:flex-nowrap gap-8 overflow-x-hidden justify-center lg:justify-start lg:overflow-x-auto pb-12 snap-x snap-mandatory">
              ...
              {[1, 2, 3, 4].map((i) => (
                <StaggerItem key={i} className="shrink-0 w-full md:w-[45vw] lg:w-[30vw] h-[500px] bg-white rounded-3xl p-8 shadow-xl border border-slate-200 snap-center relative group overflow-hidden flex flex-col justify-between">
                   <div className="absolute top-0 right-0 w-32 h-32 bg-red-500/10 rounded-bl-full -mr-8 -mt-8 transition-transform group-hover:scale-150 duration-700"></div>
                   
                   <div>
                     <div className="w-16 h-16 rounded-2xl bg-red-50 flex items-center justify-center mb-8 border border-red-100">
                       <span className="text-2xl font-black text-red-600">0{i}</span>
                     </div>
                     <h3 className="text-3xl font-black text-slate-900 mb-4 group-hover:text-red-600 transition-colors">Giai Đoạn {i}</h3>
                   ...
                   <div className="flex items-center gap-2 text-red-600 font-bold group-hover:translate-x-2 transition-transform cursor-pointer">
      ...
      <section className="py-24 bg-red-600 text-white text-center">
         <ScrollReveal animation="fade">
            <h2 className="text-4xl font-black mb-8">Triển khai Bảo mật (Security) ngay</h2>
            <Link href="/contact" className="px-10 py-4 bg-white text-red-600 font-bold rounded-xl shadow-lg hover:bg-slate-50 transition-colors">
               Liên hệ tư vấn
            </Link>
         </ScrollReveal>
      </section>
