    ...
    <div className="min-h-screen bg-[#0F172A]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {coupons.map((coupon) => {
            ...
            return (
              <div key={coupon.id} className={`bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded p-5 border transition-colors ${coupon.isActive ? 'border-white/10' : 'border-white/10 opacity-75'}`}>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-sm flex items-center justify-center ${coupon.isActive ? 'bg-emerald-100' : 'bg-white/10'}`}>
                      <Tag className={`w-5 h-5 ${coupon.isActive ? 'text-emerald-600' : 'text-slate-500'}`} />
                    </div>
                    <div>
                      <h3 className="font-bold text-white font-mono">{coupon.code}</h3>
                      <p className="text-xs text-slate-500">{coupon.description || 'Mã ưu đãi CloudHost'}</p>
                    </div>
                  </div>
                  <span className={`px-2 py-1 rounded-full text-xs font-bold ${coupon.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-white/10 text-slate-500'}`}>
                ...
                <div className="flex items-center gap-2 pt-4 border-t border-slate-100">
                  ...
                  <button className="p-2 text-slate-500 hover:text-[#1F1F1F] hover:bg-blue-900/30 rounded-sm transition-colors">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDeleteCoupon(coupon.id)} className="p-2 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-sm transition-colors">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-md p-6 max-w-md w-full">
            <h3 className="text-lg font-bold text-white mb-4">Thêm mã giảm giá mới</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-semibold text-slate-200 mb-1">Mã giảm giá</label>
                <input
                  type="text"
                  value={newCoupon.code}
                  onChange={(e) => setNewCoupon({ ...newCoupon, code: e.target.value.toUpperCase() })}
                  className="w-full px-4 py-2.5 rounded-sm border border-white/10 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 font-mono"
                  placeholder="VD: WELCOME50"
                />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-200 mb-1">Mô tả</label>
                <input
                  type="text"
                  value={newCoupon.description}
                  onChange={(e) => setNewCoupon({ ...newCoupon, description: e.target.value })}
                  className="w-full px-4 py-2.5 rounded-sm border border-white/10 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                  placeholder="Mô tả ngắn gọn..."
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-200 mb-1">Loại giảm</label>
                  <select
                    value={newCoupon.discountType}
                    onChange={(e) => setNewCoupon({ ...newCoupon, discountType: e.target.value as any })}
                    className="w-full px-3 py-2.5 rounded-sm border border-white/10 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                  >
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-200 mb-1">Giá trị giảm</label>
                  <input
                    type="number"
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-200 mb-1">Đơn hàng tối thiểu (VNĐ)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-200 mb-1">Ngày bắt đầu</label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-200 mb-1">Ngày kết thúc</label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-200 mb-1">Giới hạn sử dụng</label>
                <input
                  type="number"
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={() => setShowAddModal(false)} className="flex-1 py-2.5 rounded-sm bg-white/10 text-slate-200 font-semibold text-sm hover:bg-white/20 transition-colors">
                Hủy
              </button>
            </div>
          </div>
