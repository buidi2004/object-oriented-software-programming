    ...
    <div className="min-h-screen bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">{tickets.length} tickets tổng cộng</p>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="bg-white rounded p-4 border border-slate-200 mb-6 flex flex-wrap gap-4 items-center">
          ...
          <select
            ...
            className="px-3 py-2.5 rounded-sm border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white"
          >
                ...
                {filteredTickets.map((ticket) => (
                  <tr key={ticket.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      ...
                      <p className="text-xs text-slate-600 mt-0.5">{ticket.messageCount || 1} tin nhắn</p>
                    </td>
                    <td className="py-3 px-4">
                      <p className="font-medium text-slate-900">{ticket.customerName || 'Khách hàng'}</p>
                      <p className="text-xs text-slate-600">{ticket.customerEmail}</p>
                    </td>
                    <td className="py-3 px-4">
                      <span className="px-2 py-1 rounded-sm bg-slate-100 text-xs font-medium text-slate-700">
                    ...
                    <td className="py-3 px-4 text-right flex justify-end gap-2">
                      <button 
                        ...
                        className="inline-flex p-2 text-slate-600 hover:text-emerald-600 hover:bg-emerald-50 rounded-sm transition-colors"
                        title="Gửi Email cho khách hàng"
                      >
                        <Mail className="w-4 h-4" />
                      </button>
                      <Link href={`/tickets/${ticket.id}`} className="inline-flex p-2 text-slate-600 hover:text-[#1F1F1F] hover:bg-blue-50 rounded-sm transition-colors">
      ...
      {emailModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/50 backdrop-blur-sm">
          <div className="bg-white rounded shadow-xl w-full max-w-2xl overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
              ...
              <button 
                ...
                className="p-2 text-slate-600 hover:text-slate-600 hover:bg-slate-200 rounded-sm transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSendEmail} className="p-6 flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-sm border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-sm border border-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 min-h-[200px]"
              ...
              <div className="flex items-center justify-end gap-3 mt-4">
                <button
                  type="button"
                  onClick={() => setEmailModalOpen(false)}
                  className="px-4 py-2 rounded-sm text-slate-600 font-medium hover:bg-slate-100 transition-colors"
                >
                  Hủy
                </button>
                <button
