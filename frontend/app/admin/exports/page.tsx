  ...
  return (
    <div className="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-6xl mx-auto">
        {/* Navigation & Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-white p-6 rounded-xl border border-slate-200 shadow-2xs">
          <div>
          ...
          {/* Date Range Selector */}
          <div className="flex items-center gap-2.5 self-start md:self-auto bg-slate-50 p-1.5 rounded-lg border border-slate-200">
            <Calendar className="w-4 h-4 text-slate-500 ml-2" />
            <select
              value={dateRange}
              onChange={(e) => setDateRange(e.target.value as any)}
              className="px-3 py-1.5 rounded-md border-0 text-xs font-bold bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-2xs cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {exportCategories.map((item) => (
            <div
              key={item.id}
              className="bg-white rounded-xl p-5 sm:p-6 border border-slate-200/90 shadow-2xs hover:shadow-md transition-all flex flex-col justify-between"
            >
              <div>
                <div className="flex items-center justify-between mb-3.5">
                  ...
                  <button
                    ...
                    className="text-[11px] font-bold text-slate-500 hover:text-blue-600 bg-slate-50 hover:bg-slate-100 px-2.5 py-1 rounded-md transition-colors flex items-center gap-1 border border-slate-200/60"
                  >
              ...
              <div className="pt-4 border-t border-slate-100 space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-1 py-1.5 px-2.5 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-[11px] transition-all flex items-center justify-center gap-1 border border-slate-200 shadow-2xs disabled:opacity-50"
      ...
      {previewData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white rounded-2xl w-full max-w-5xl max-h-[85vh] flex flex-col shadow-2xl animate-in zoom-in-95 overflow-hidden">
            {/* Modal Header */}
                    ...
                    <thead className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200">
                      <tr>
                        {previewData.category.columns.map((col, idx) => (
                          <th key={idx} className="p-2.5 whitespace-nowrap border-r border-slate-200 last:border-0">
                            {col.header}
                          </th>
                        ))}
                      </tr>
