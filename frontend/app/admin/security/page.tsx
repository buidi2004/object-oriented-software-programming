...
export default function AdminSecurityAddonsPage() {
  ...
  const getStatusBadge = (status: number) => {
    switch (status) {
      case 1: return <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-bold bg-blue-900/50 text-[#1F1F1F]"><RefreshCw className="w-3 h-3 animate-spin"/> Scanning</span>;
      ...
      <div className="min-h-screen bg-[#0F172A] py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto space-y-6">
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={fetchAddons}
              className="p-2 rounded bg-[#1E293B] bg-opacity-70 backdrop-blur-md border border-white/10 hover:text-emerald-600 transition-colors"
            >
              ...
              <table className="w-full text-left text-sm text-slate-500">
                ...
                <tbody className="divide-y divide-white/10">
                  {addons.length === 0 ? (
                    ...
                    addons.map((addon) => (
                      <tr key={addon.id} className="hover:bg-[#0F172A]/50">
                        <td className="p-4 text-xs font-mono">{addon.userId}</td>
                        <td className="p-4 font-bold text-white">{addon.targetResourceId}</td>
