      ...
      <header className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md border-b border-white/10 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">Tối Ưu Hóa SEO Dịch Vụ (On-Page SEO)</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="space-y-6">
          {filteredServices.map((service) => {
            ...
            return (
              <div key={service.id} className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-lg border border-white/10 p-6 sm:p-8 shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-md bg-blue-900/30 text-[#1F1F1F] flex items-center justify-center font-bold">
                  ...
                  <div>
                    {!isEditing ? (
                      <button
                        ...
                        className="px-4 py-2 rounded bg-blue-900/30 hover:bg-blue-900/50 text-[#1F1F1F] font-bold text-xs transition-colors flex items-center gap-1.5"
                      >
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 rounded bg-white/10 hover:bg-white/20 text-slate-500 font-bold text-xs transition-colors"
                      >
                        Đóng
                      </button>
                    )}
                ...
                {isEditing ? (
                  <div className="mt-6 space-y-4 animate-in fade-in-50">
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1.5">
                        <label className="text-xs font-bold text-slate-200 uppercase">Tiêu đề Trang (Meta Title)</label>
                        <span className={`text-[10px] font-bold ${formData.metaTitle.length > 60 ? 'text-amber-600' : 'text-slate-500'}`}>
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-1.5">
                        <label className="text-xs font-bold text-slate-200 uppercase">Mô tả Trang (Meta Description)</label>
                        <span className={`text-[10px] font-bold ${formData.metaDescription.length > 160 ? 'text-amber-600' : 'text-slate-500'}`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-bold text-slate-200 uppercase mb-1.5">Từ Khóa (Keywords)</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-xs font-bold text-slate-200 uppercase mb-1.5">Ảnh Đại Diện Chia Sẻ (OpenGraph Image URL)</label>
                        <input
                          type="text"
                    ...
                    <div className="bg-[#0F172A] p-4 rounded-md border border-white/10 mt-4">
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    <div className="bg-[#0F172A] p-4 rounded-md border border-slate-100">
                      <div className="font-bold text-slate-200 mb-1">Tiêu Đề SEO Hiện Tại:</div>
                      <div className="text-white font-semibold">{currentSeo?.metaTitle || service.name}</div>
                      <div className="font-bold text-slate-200 mt-3 mb-1">Mô Tả Meta:</div>
                      <div className="text-slate-500 leading-relaxed">{currentSeo?.metaDescription || 'Mô tả mặc định tự động'}</div>
                    </div>

                    <div className="bg-[#0F172A] p-4 rounded-md border border-slate-100 flex flex-col justify-between">
                      <div>
                        <div className="font-bold text-slate-200 mb-1">Từ Khóa:</div>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {currentSeo?.keywords?.split(',').map((kw, i) => (
                            <span key={i} className="px-2 py-0.5 rounded-sm bg-[#1E293B] bg-opacity-70 backdrop-blur-md border border-white/10 text-[10px] text-slate-200 font-medium">
                              {kw.trim()}
                            </span>
                          )) || <span className="text-slate-500">Chưa đặt từ khóa</span>}
                        </div>
                      </div>
