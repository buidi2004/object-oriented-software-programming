    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900">Quản lý Danh mục Dịch vụ</h1>
              <p className="text-xs text-slate-600">{categories.length} danh mục có sẵn</p>
            </div>
          </div>
          <button 
            onClick={() => {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="mb-6 max-w-md relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 shadow-sm"
          />
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="text-left py-3.5 px-4 font-semibold text-slate-700">Tên danh mục</th>
                  <th className="text-left py-3.5 px-4 font-semibold text-slate-700">Slug URL</th>
                  <th className="text-left py-3.5 px-4 font-semibold text-slate-700">Mô tả</th>
                  <th className="text-left py-3.5 px-4 font-semibold text-slate-700">Số gói dịch vụ</th>
                  <th className="text-left py-3.5 px-4 font-semibold text-slate-700">Ngày tạo</th>
                  <th className="text-right py-3.5 px-4 font-semibold text-slate-700">Thao tác</th>
                </tr>
              </thead>
                  ...
                  <tr key={category.id} className="hover:bg-slate-50/80 transition-colors">
                    <td className="py-3.5 px-4">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-sm bg-blue-50 text-[#1F1F1F] flex items-center justify-center font-bold">
                          <FolderTree className="w-4 h-4" />
                        </div>
                        <span className="font-semibold text-slate-900">{category.name}</span>
                      </div>
                    </td>
                    <td className="py-3.5 px-4 font-mono text-xs text-slate-600">{category.slug}</td>
                    <td className="py-3.5 px-4 text-slate-600 max-w-xs truncate">{category.description}</td>
                    <td className="py-3.5 px-4">
                      <span className="px-2.5 py-1 rounded-full bg-slate-100 text-xs font-semibold text-slate-700">
                        {category.serviceCount} gói
                      </span>
                    </td>
                    <td className="py-3.5 px-4 text-slate-600 text-xs">{new Date(category.createdAt).toLocaleDateString('vi-VN')}</td>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-md p-6 max-w-md w-full shadow-2xl">
            ...
            <form onSubmit={handleCreateCategory} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1.5">Tên danh mục</label>
                <input
                  type="text"
                  required
                  value={formData.name}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1.5">Slug URL</label>
                <input
                  type="text"
                  required
                  value={formData.slug}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 uppercase mb-1.5">Mô tả</label>
                <textarea
                  value={formData.description}
                  onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                  rows={3}
                  className="w-full px-4 py-2.5 rounded border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 resize-none"
                  placeholder="Mô tả ngắn gọn về danh mục dịch vụ..."
                />
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowAddModal(false)} className="flex-1 py-2.5 rounded bg-slate-100 text-slate-700 font-semibold text-sm hover:bg-slate-200 transition-colors">
                  Hủy
                </button>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-md p-6 max-w-md w-full shadow-2xl">
            ...
            <form onSubmit={handleUpdateCategory} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowEditModal(false)} className="flex-1 py-2.5 rounded bg-slate-100 text-slate-700 font-semibold text-sm hover:bg-slate-200 transition-colors">
                  Hủy
                </button>
                <button type="submit" className="flex-1 py-2.5 rounded bg-blue-600 text-white font-semibold text-sm hover:bg-blue-700 transition-colors">
                  Cập Nhật Thay Đổi
