        ...
        <div className="bg-white rounded-md border border-slate-200 p-6 shadow-sm">
          ...
          {status === 'pending' && (
            <div className="flex gap-3 mt-6 pt-6 border-t border-slate-100">
              <button onClick={() => handleAction('approve')} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-sm font-semibold text-sm">
                <CheckCircle2 className="w-4 h-4" /> Duyệt
              </button>
              <button onClick={() => handleAction('reject')} className="px-4 py-2 bg-red-600 text-white rounded-sm font-semibold text-sm">
                Từ chối
              </button>
            </div>
          )}
