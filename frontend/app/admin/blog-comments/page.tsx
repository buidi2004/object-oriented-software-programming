  ...
  return (
    <div className="min-h-screen bg-[#0F172A]">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-xl font-bold text-white">Quản lý Bình luận Tin tức &amp; Đánh Giá</h1>
                <p className="text-xs text-slate-500">{comments.length} bình luận bài viết trên hệ thống</p>
              </div>
            </div>
            <button 
              onClick={fetchComments}
              className="px-4 py-2 rounded-sm bg-white/10 text-slate-200 font-semibold text-xs hover:bg-white/20 transition-colors flex items-center gap-2"
            >
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-slate-100">
            <Link
              href="/admin/reviews"
              className="px-4 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-slate-200 text-xs font-bold flex items-center gap-2 transition-colors border border-white/10/80"
            >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded p-4 border border-white/10 mb-6">
        ...
        <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded border border-white/10 overflow-hidden shadow-sm">
          <div className="divide-y divide-white/10">
            {filteredComments.map((comment) => (
              <div key={comment.id} className="p-5 hover:bg-[#0F172A] transition-colors">
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1.5 flex-1">
                    ...
                    <p className="text-slate-200 text-sm leading-relaxed bg-[#0F172A]/80 p-3 rounded border border-slate-100">
                      {comment.content}
                    </p>
                  </div>
