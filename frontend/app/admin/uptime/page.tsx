      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {systemStatus && (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-md p-5 border border-white/10 shadow-sm">
            ...
            <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-md p-5 border border-white/10 shadow-sm">
              ...
              <div className="text-2xl font-black text-white">{systemStatus.uptime}%</div>
              <div className="text-[10px] text-slate-500 mt-1">Cam kết SLA: 99.9%</div>
            </div>
            ...
            <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-md p-5 border border-white/10 shadow-sm">
              ...
              <div className="text-[10px] text-slate-500 mt-1">Đo đạc từ 3 miền Bắc - Trung - Nam</div>
            </div>
            ...
            <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-md p-5 border border-white/10 shadow-sm">
              ...
              <div className="text-[10px] text-slate-500 mt-1">100% Requests được bảo vệ qua WAF</div>
            </div>
          </div>
        )}
        ...
        <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-lg border border-white/10 overflow-hidden shadow-sm">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/10">
              {services.map((svc) => (
                <tr key={svc.id} className="hover:bg-[#0F172A] transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded bg-white/10 text-slate-200 flex items-center justify-center font-bold">
                        <Server className="w-5 h-5" />
                      </div>
                      <div>
                        <p className="font-bold text-white text-sm">{svc.name}</p>
                        <p className="text-xs text-slate-500 font-mono">{svc.url}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    {svc.isPaused ? (
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-white/10 text-slate-500">
                  ...
                  <td className="px-6 py-4 font-mono font-black text-slate-100">
                    {svc.uptime}%
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1.5">
                      <button
                        ...
                        className="px-2.5 py-1.5 rounded bg-white/10 hover:bg-white/20 text-slate-200 font-bold text-xs flex items-center gap-1 transition-colors disabled:opacity-50"
                      ...
                      <button
                        ...
                        className="p-1.5 text-slate-500 hover:text-slate-200 hover:bg-white/10 rounded-sm transition-colors"
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-lg p-6 sm:p-8 max-w-md w-full shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => { setShowAddModal(false); setEditingService(null); }} className="p-1.5 text-slate-500 hover:text-slate-500 rounded-sm hover:bg-white/10">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={editingService ? handleUpdateService : handleAddService} className="space-y-3">
                <div>
                  <label className="block text-xs font-bold text-slate-200 mb-1">Tên Dịch Vụ / Server</label>
                  <input
                    type="text"
                    required
                    value={formData.name}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-200 mb-1">Địa Chỉ URL / IP Health Check</label>
                  <input
                    type="text"
                    required
                    value={formData.url}
