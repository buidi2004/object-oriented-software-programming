  ...
  return (
    <div className="min-h-screen bg-[#0F172A]">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2.5">
            <Link
              href="/faqs"
              target="_blank"
              className="px-3.5 py-2 rounded-lg bg-white/10 text-slate-200 hover:bg-white/20 font-bold text-xs transition-colors flex items-center gap-1.5"
            >
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-xl p-4 border border-white/10/90 shadow-2xs mb-6 space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 rounded-lg border border-white/10 text-xs text-white focus:outline-none focus:ring-2 focus:ring-blue-500 bg-[#0F172A]/50"
              />
            </div>
            
            <button
              onClick={() => fetchFaqs()}
              className="px-3 py-2 text-xs font-bold text-slate-500 hover:text-white bg-white/10 rounded-lg hover:bg-white/20 transition-colors flex items-center gap-1.5 shrink-0 self-end sm:self-auto"
            >
        ...
        <div className="space-y-3">
          {filteredFaqs.map((faq) => (
            <div 
              key={faq.id} 
              className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-xl border border-white/10/90 hover:border-white/20 shadow-2xs transition-all overflow-hidden"
            >
              <div
                onClick={() => setOpenId(openId === faq.id ? null : faq.id)}
                className="w-full p-4 flex items-center justify-between text-left hover:bg-[#0F172A]/50 cursor-pointer transition-colors"
              >
                <div className="flex items-center gap-3.5 flex-1 min-w-0">
                  <div className="w-8 h-8 rounded-lg bg-blue-900/30 text-blue-400 flex items-center justify-center shrink-0 border border-blue-100">
                  ...
                  <div className="flex-1 min-w-0 pr-4">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="px-2 py-0.5 rounded-md bg-white/10 text-[11px] font-bold text-slate-500 flex items-center gap-1 border border-white/10/60">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {openId === faq.id ? <ChevronUp className="w-4 h-4 text-slate-500" /> : <ChevronDown className="w-4 h-4 text-slate-500" />}
                </div>
              </div>
              
              {openId === faq.id && (
                <div className="px-5 pb-4.5 border-t border-slate-100 pt-3.5 bg-[#0F172A]/40">
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-white/10/60">
                    ...
                    <div className="flex items-center gap-2">
                      <button 
                        ...
                        className="px-3 py-1.5 rounded-lg bg-blue-900/30 text-blue-300 border border-blue-200 hover:bg-blue-900/50 text-xs font-bold flex items-center gap-1.5 transition-colors"
                      >
        ...
        {filteredFaqs.length === 0 && (
          <div className="text-center py-16 bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-xl border border-white/10 shadow-2xs">
            <AlertCircle className="w-10 h-10 mx-auto mb-2 text-slate-500" />
            <p className="font-bold text-white text-sm">Không tìm thấy câu hỏi FAQ nào</p>
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-xl w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-[#0F172A]/50">
              <h2 className="text-base font-black text-white flex items-center gap-2">
                <Plus className="w-4 h-4 text-blue-400" />
                <span>Thêm Câu Hỏi FAQ Mới</span>
              </h2>
              <button onClick={() => setIsCreateModalOpen(false)} className="p-1 rounded-lg hover:bg-white/20 text-slate-500">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateFaq} className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-200 mb-1.5">Câu hỏi FAQ *</label>
                <input
                  type="text"
                  required
                  value={faqForm.question}
                  onChange={e => setFaqForm({...faqForm, question: e.target.value})}
                  className="w-full px-3.5 py-2 text-xs rounded-lg border border-white/10 focus:outline-none focus:ring-2 focus:ring-blue-500 text-white"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={5}
                  className="w-full px-3.5 py-2 text-xs rounded-lg border border-white/10 focus:outline-none focus:ring-2 focus:ring-blue-500 text-white resize-none"
              ...
              <div className="grid grid-cols-2 gap-3.5">
                <div>
                  <label className="block text-xs font-bold text-slate-200 mb-1.5">Danh mục (Tag)</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-200 mb-1.5">Thứ tự hiển thị</label>
                  <input
                    type="number"
                    required
                    min={1}
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2.5">
                <button
                  ...
                  className="px-4 py-2 text-xs font-bold text-slate-500 hover:bg-white/10 rounded-lg transition-colors"
                >
                  Hủy
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-xl w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95">
            <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-[#0F172A]/50">
              ...
              <button onClick={() => setIsEditModalOpen(false)} className="p-1 rounded-lg hover:bg-white/20 text-slate-500">
                <X className="w-4 h-4" />
              </button>
            </div>
      ...
      {isDeleteModalOpen && activeFaq && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 p-6">
            <div className="w-12 h-12 rounded-full bg-rose-100 text-rose-600 flex items-center justify-center mx-auto mb-4">
              <Trash2 className="w-6 h-6" />
            </div>
            
            <h3 className="text-base font-black text-white text-center">Xóa Câu Hỏi FAQ?</h3>
            <p className="text-xs text-slate-500 text-center mt-2 leading-relaxed">
              Bạn có chắc chắn muốn xóa câu hỏi: <strong className="text-slate-100 font-bold">"{activeFaq.question}"</strong>?
