      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2.5 rounded bg-[#1E293B] bg-opacity-70 backdrop-blur-md border border-white/10 text-xs text-white focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm w-64"
              />
            </div>
            <button
              onClick={() => {
        ...
        <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-lg border border-white/10 shadow-sm overflow-hidden mb-8">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead className="bg-[#0F172A] border-b border-white/10 text-slate-500 font-bold uppercase tracking-wider">
                <tr>
              ...
              <tbody className="divide-y divide-white/10">
                {filtered.map((dom) => (
                  <tr key={dom.id} className="hover:bg-[#0F172A]/80 transition-colors">
                    ...
                    <td className="py-4 px-6 text-slate-500 font-mono">{dom.ownerEmail}</td>
                    <td className="py-4 px-6 text-slate-500">{dom.registrar}</td>
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded bg-white/10 hover:bg-white/20 text-slate-200 font-bold text-[11px] flex items-center gap-1 transition-colors"
                        >
        ...
        {selectedEpp && (
          <div className="fixed inset-0 bg-[#1E293B] bg-opacity-70 backdrop-blur-md/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-lg p-6 max-w-sm w-full border border-white/10 shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-base font-black text-white">Mã EPP Auth Code</h2>
                <button onClick={() => setSelectedEpp(null)} className="p-1.5 text-slate-500 hover:text-slate-500 rounded-sm hover:bg-white/10">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <p className="text-xs text-slate-500 mb-3">Tên miền: <strong>{selectedEpp.domain}</strong></p>
              <div className="p-3 bg-[#0F172A] border border-white/10 rounded-md flex items-center justify-between font-mono text-xs font-bold text-white mb-4">
                <span>{selectedEpp.code}</span>
                <button
                  onClick={() => copyEpp(selectedEpp.code)}
                  className="p-1 text-slate-500 hover:text-[#1F1F1F] rounded-sm hover:bg-[#1E293B] bg-opacity-70 backdrop-blur-md transition-colors"
                >
              ...
              <button
                ...
                className="w-full py-2.5 rounded bg-white/10 hover:bg-white/20 text-slate-200 text-xs font-bold"
              >
                Đóng
              </button>
            </div>
          ...
          <div className="fixed inset-0 bg-[#1E293B] bg-opacity-70 backdrop-blur-md/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#1E293B] bg-opacity-70 backdrop-blur-md rounded-lg p-6 sm:p-8 max-w-lg w-full border border-white/10 shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={() => { setShowAddModal(false); setEditingDomain(null); }} className="p-1.5 text-slate-500 hover:text-slate-500 rounded-sm hover:bg-white/10">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={editingDomain ? handleUpdate : handleCreate} className="space-y-3">
                <div>
                  <label className="block text-xs font-bold text-slate-200 mb-1">Tên Miền</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-3 py-2 text-xs rounded border border-white/10 focus:ring-2 focus:ring-blue-500 outline-none font-mono disabled:bg-white/10"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 text-xs rounded border border-white/10 focus:ring-2 focus:ring-blue-500 outline-none"
                  />
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-bold text-slate-200 mb-1">Nhà Đăng Ký (Registrar)</label>
                    <select
                      value={formData.registrar}
                      onChange={e => setFormData({ ...formData, registrar: e.target.value })}
                      className="w-full px-3 py-2 text-xs rounded border border-white/10 focus:ring-2 focus:ring-blue-500 outline-none bg-[#1E293B] bg-opacity-70 backdrop-blur-md"
                    >
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-200 mb-1">Thời Hạn (Năm)</label>
                    <input
                      type="number"
                      required
                      min={1}
                      ...
                      className="w-full px-3 py-2 text-xs rounded border border-white/10 focus:ring-2 focus:ring-blue-500 outline-none disabled:bg-white/10"
                    />
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-200 mb-1">Nameservers (DNS)</label>
                  <input
                    type="text"
                    ...
                    className="w-full px-3 py-2 text-xs rounded border border-white/10 focus:ring-2 focus:ring-blue-500 outline-none font-mono"
                  />
                </div>
