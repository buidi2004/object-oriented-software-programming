    ...
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200">
        <div className="max-w-3xl mx-auto px-4 h-16 flex items-center gap-4">
          <Link href="/admin/migrations" className="p-2 rounded-sm hover:bg-slate-100">
        ...
        <div className="bg-white rounded-md border border-slate-200 p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">Order: {migration.orderRequestId}</p>
            </div>
          </div>
          ...
          <p className="text-sm text-slate-600">Tạo lúc: {new Date(migration.createdAt).toLocaleString('vi-VN')}</p>
          ...
          <div className="flex gap-2 pt-4 border-t border-slate-100">
            {[1, 2, 3].map((s) => (
              <button
                ...
                className="px-3 py-1.5 text-xs font-semibold bg-slate-100 hover:bg-slate-200 rounded-sm"
              >
                Status {s}
              </button>
            ))}
