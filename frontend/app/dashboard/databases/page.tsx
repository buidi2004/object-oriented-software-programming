        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-slate-900 flex items-center gap-3">
              <div className="p-2.5 rounded-md bg-teal-100 text-teal-600">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2.5 rounded-md bg-white border border-slate-200 text-slate-600 hover:text-teal-600 hover:border-teal-300 transition-all shadow-sm"
              title="Làm mới"
            >
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-md bg-teal-600 hover:bg-teal-700 text-slate-900 font-bold text-xs sm:text-sm transition-all shadow-lg shadow-teal-500/25 flex items-center gap-2"
            >
        ...
        <div className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
          ...
          {loading && databases.length === 0 ? (
            ...
            <div className="p-12 text-center">
              <div className="w-16 h-16 rounded-lg bg-teal-50 text-teal-500 flex items-center justify-center mx-auto mb-4">
              ...
              <button
                onClick={() => setIsCreateOpen(true)}
                className="px-5 py-2.5 rounded-md bg-teal-600 text-slate-900 font-bold text-xs shadow-md"
              >
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 rounded border border-slate-200 text-xs text-slate-900 outline-none focus:ring-2 focus:ring-teal-500 bg-white font-bold"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded border border-slate-200 text-xs text-slate-900 outline-none focus:ring-2 focus:ring-teal-500"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={creating}
                  className="px-5 py-2.5 rounded bg-teal-600 hover:bg-teal-700 text-slate-900 font-bold text-xs shadow-md flex items-center gap-2"
                >
    ...
    <div className="p-6 hover:bg-slate-50/60 transition-colors space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-start sm:items-center gap-3">
          <div className="w-10 h-10 rounded-md bg-teal-100 text-teal-600 flex items-center justify-center flex-shrink-0">
            <Database className="w-5 h-5" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="font-black text-slate-900 text-sm">{db.name}</span>
              <span className="text-[10px] uppercase font-extrabold px-2 py-0.5 rounded-sm bg-slate-100 text-slate-600">
        ...
        <div className="flex items-center gap-3">
          ...
          {status === 'Running' || status === 'Active' ? (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded bg-slate-100 hover:bg-teal-50 text-slate-700 hover:text-teal-700 font-bold text-xs transition-colors"
            >
      ...
      {(status === 'Running' || status === 'Active') && db.adminPassword && (
        <div className="flex items-center gap-2 pt-2 border-t border-slate-100">
          <span className="text-xs text-slate-600 font-medium">Mật khẩu Admin:</span>
