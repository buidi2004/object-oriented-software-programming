  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link href="/dashboard/ssl-certificates" className="p-2 rounded-lg hover:bg-slate-100">
          <ArrowLeft className="w-5 h-5 text-slate-600" />
        </Link>
        <div>
          <h1 className="text-2xl font-black text-slate-900">Chi tiết SSL Certificate</h1>
          <p className="text-slate-500 mt-1">{cert.domain?.name || cert.domainName || 'Chứng chỉ SSL'}</p>
        </div>
      </div>
        ...
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 font-mono">ID: {cert.id}</p>
            </div>
          </div>
        ...
        {cert.csr && (
          <div className="bg-slate-50 rounded-xl p-4 mb-4 border border-slate-100">
            <div className="flex justify-between items-center mb-2">
              ...
              <button onClick={() => copyToClipboard(cert.csr)} className="text-slate-400 hover:text-blue-600">
        ...
        {cert.certificate && (
          <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-4">
            ...
            <div className="col-span-2 flex gap-2 mt-2">
              <button className="flex-1 bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center justify-center gap-2">
