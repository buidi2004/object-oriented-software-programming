  ...
  if (error || !ticket) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh]">
        ...
        <Link href="/dashboard/tickets" className="px-6 py-2 bg-slate-200 hover:bg-slate-300 rounded-sm font-bold transition-colors">
          Quay lại danh sách
        </Link>
      </div>
    );
    ...
    <div className="py-6">
      <div className="max-w-4xl mx-auto w-full px-4">
        ...
        <div className="flex items-center justify-between mb-4 shrink-0">
          <Link href="/dashboard/tickets" className="inline-flex items-center gap-2 text-slate-600 hover:text-slate-900 font-medium">
          ...
          {!isClosed && (
            <button 
              ...
              disabled={isClosing}
              className="text-sm font-bold text-rose-600 hover:bg-rose-50 px-3 py-1.5 rounded-sm border border-rose-200 transition-colors flex items-center gap-1.5"
            >
        ...
        <div className="bg-white rounded-t-md border border-slate-200 p-5 shadow-sm shrink-0">
          ...
          {ticket.messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                ...
                <div 
                  className={`max-w-[85%] md:max-w-[75%] p-4 rounded-md ${
                    isMe 
        ...
        {/* Message Input Box */}
        <div className="bg-white rounded-b-md border border-t-0 border-slate-200 p-4 shrink-0">
          {isClosed ? (
            <div className="text-center py-3 text-slate-600 text-sm flex items-center justify-center gap-2">
            ...
            <form onSubmit={handleReply} className="flex flex-col sm:flex-row items-end sm:items-center gap-3">
              <div className="flex-1 w-full bg-slate-50 rounded border border-slate-200 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-500/20 transition-all">
                <textarea
                  value={replyText}
              ...
              <button 
                ...
                className="w-full sm:w-auto shrink-0 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white p-3 rounded flex items-center justify-center gap-2 font-bold transition-colors"
              >
