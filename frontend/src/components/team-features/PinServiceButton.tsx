...
export function PinServiceButton({ serviceType, serviceId, displayName }: { serviceType: 'VPS' | 'DOMAIN' | 'SSL'; serviceId: string; displayName: string }) {
  ...
  return <button type="button" onClick={toggle} disabled={busy} title={pinned ? 'Bỏ ghim' : 'Ghim vào Quick Access'} className={'p-2 rounded-sm border transition-colors ' + (pinned ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white border-slate-200 text-slate-600 hover:text-[#1F1F1F]')}><Pin className={'w-4 h-4 ' + (pinned ? 'fill-current' : '')}/></button>;
}
