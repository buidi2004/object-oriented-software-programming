    ...
    <section className="py-16 bg-slate-50 border-t border-slate-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          ...
          <Link href="/services" className="text-sm font-bold text-slate-600 hover:text-[#1F1F1F] transition-colors">
            Xem tất cả dịch vụ →
          </Link>
        </div>
          ...
          <div className="w-full lg:w-64 shrink-0 flex lg:flex-col overflow-x-auto lg:overflow-visible gap-2 lg:gap-1 border-b lg:border-b-0 lg:border-r border-slate-200 pb-3 lg:pb-0 lg:pr-4 scrollbar-none">
            {tabs.map((tab) => (
              <button
                ...
                className={`whitespace-nowrap lg:whitespace-normal text-left px-4 py-2.5 lg:py-3 rounded text-xs sm:text-sm font-bold transition-all shrink-0 lg:shrink ${
                  activeTab === tab.id
                    ? 'bg-[#1F1F1F] text-white lg:bg-slate-100 lg:text-[#1F1F1F] shadow-sm lg:shadow-none'
          ...
          <div className="flex-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
              {cloudServices.map((service, idx) => {
                ...
                return (
                  <div key={idx} className="bg-white rounded-md p-5 sm:p-6 border border-slate-100 shadow-xs hover:shadow-md transition-shadow group flex flex-col justify-between">
                    <div>
                      <div className="flex items-start gap-3.5 sm:gap-4 mb-3 sm:mb-4">
                        <div className="w-11 h-11 sm:w-12 sm:h-12 rounded bg-slate-100 text-[#1F1F1F] flex items-center justify-center shrink-0">
