...
public class ConfirmPaymentWebhookCommandHandler : IRequestHandler<ConfirmPaymentWebhookCommand>
{
    ...
    public async Task Handle(ConfirmPaymentWebhookCommand request, CancellationToken ct)
    {
        ...
        var lockKey = string.IsNullOrWhiteSpace(rawKey) ? Guid.NewGuid().ToString() : rawKey;
        ...
        try
        {
            ...
            {
                payment = await _paymentRepo.FirstOrDefaultAsync(p => p.Status == PaymentStatus.Pending && 
                    (p.IdempotencyKey.Contains(prefix) || p.OrderId.ToString().Contains(prefix)), ct);
            }

            if (payment == null)
            {
                var directOrder = await _orderRepo.FirstOrDefaultAsync(o => o.Status == OrderStatus.Pending &&
                    ((parsedGuid != Guid.Empty && o.Id == parsedGuid) ||
                    (prefix.Length >= 6 && o.Id.ToString().Contains(prefix))), ct);
            ...
            if (payment.Amount > 0 && request.Amount > 0 && payment.Amount != request.Amount)
            {
                // Handle unit mismatch (e.g. 4320k VND vs 4320000 VND)
                bool isUnitMismatch = (payment.Amount == request.Amount * 1000m) || (request.Amount == payment.Amount * 1000m);
                // Handle VAT/tax difference (e.g. 4,320,000 + 8% VAT = 4,665,600)
                decimal ratio = payment.Amount > 0 ? (request.Amount / payment.Amount) : 1m;
                bool isVatOrFeeDiff = ratio >= 0.8m && ratio <= 1.25m;

                if (!isUnitMismatch && !isVatOrFeeDiff && Math.Abs(payment.Amount - request.Amount) > 100000m)
                {
