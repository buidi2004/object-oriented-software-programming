...
public class AddToCartCommandHandlerTests
{
    ...
    public async Task Handle_UserNotLoggedIn_ThrowsUnauthorizedException()
    {
        ...
        await Assert.ThrowsAsync<UnauthorizedException>(
            () => CreateHandler().Handle(new AddToCartCommand(Guid.NewGuid(), "Monthly", 1), CancellationToken.None));
    }
    ...
    public async Task Handle_CartDoesNotExist_CreatesCartAndAddsItem()
    {
        ...
        await CreateHandler().Handle(new AddToCartCommand(planId, "Monthly", 2), CancellationToken.None);

        _cartRepoMock.Verify(r => r.AddAsync(It.Is<Cart>(c => c.UserId == userId && c.Status == "Active"), It.IsAny<CancellationToken>()), Times.Once);
    ...
    public async Task Handle_ItemAlreadyInCart_UpdatesQuantity()
    {
        var userId = Guid.NewGuid();
        var planId = Guid.NewGuid();
        var cart = new Cart { Id = Guid.NewGuid(), UserId = userId, Status = "Active" };
        var existingItem = new CartItem { Id = Guid.NewGuid(), CartId = cart.Id, ServicePlanId = planId, BillingCycle = "Monthly", Quantity = 1 };
        ...
        await CreateHandler().Handle(new AddToCartCommand(planId, "Monthly", 3), CancellationToken.None);
